...
#define MPU6050							//ʹõĴΪMPU6050
#define MOTION_DRIVER_TARGET_MSP430		//,MSP430(ֲSTM32F1)
...
//}
#define log_i 	printf	//ӡϢ
#define log_e  	printf	//ӡϢ
...
u8 run_self_test(void)
{
	...
	if (result == 0x3) 
	{
		...
		*/
		...
		//accel_sens=0;//注释掉  防止零位小角度平衡，让MPU6050输出值为0  --zzw 2024.6.10
...
u8 mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//设置陀螺仪方向矩阵
		if(res)return 5; 
...
static KalmanFilter_mpu6050 Kalman_Struct[3] = {{0.02,0,0,0,0.001,0.8},{0.02,0,0,0,0.001,0.8},{0.02,0,0,0,0.001,0.8}};

static volatile int16_t gyro_databuff[3]={0};//角速度数据暂存


void kalman_Filter_1(KalmanFilter_mpu6050 *ekf,float input)  //一维卡尔曼
{
...
static float calibration_value_accel[3]={0,0,0};//角加速度校准值
...
u8 MPU_Get_Accelerometer(float *ax,float *ay,float *az)
{
    ...
    u8 buf[6],res;  
    static int16_t accel_databuff[3]={0};//角加速度数据暂存
    ...
    /*对加速度进行卡尔曼滤波*/
    #if 1
        kalman_Filter_1(&Kalman_Struct[0],(float)accel_databuff[0]);
        accel_databuff[0]=(int16_t)Kalman_Struct[0].out;
        kalman_Filter_1(&Kalman_Struct[1],(float)accel_databuff[1]);
        accel_databuff[1]=(int16_t)Kalman_Struct[1].out;
        kalman_Filter_1(&Kalman_Struct[2],(float)accel_databuff[2]);
        accel_databuff[2]=(int16_t)Kalman_Struct[2].out;
    #endif
    #if 1
        static float last_accel[3]={0};
        accel_databuff[0] = accel_databuff[0]*0.3 + last_accel[0]*0.7;
        accel_databuff[1] = accel_databuff[1]*0.3 + last_accel[1]*0.7;
        accel_databuff[2] = accel_databuff[2]*0.3 + last_accel[2]*0.7;
        last_accel[0] = accel_databuff[0];
        last_accel[1] = accel_databuff[1];
        last_accel[2] = accel_databuff[2];
    #endif
...
void MPU6050_Calibration(float aacx,float aacy,float aacz,short gyrox,short gyroy,short gyroz)//mpu6050校正函数
{
  ...
  if(Calibration_times==300)
  {
    ...
    calibration_value_accel[0] = Aacx;
    calibration_value_accel[1] = Aacy;
    calibration_value_accel[2] = Aacz;
